...
/** Signed-out demo Activity. Seeded data only — never rendered for signed-in users. */
export default function ActivityDemo() {
  const [query, setQuery] = useState("");
  const [kind, setKind] = useState<"all" | "spending" | "income" | "transfer" | "pending">("all");
  ...
  const rows = useMemo(() => {
    ...
    return demoTransactions.filter((t) => {
      ...
      if (kind === "pending" && !t.pending) return false;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2" role="group" aria-label="Filter by type">
        {(["all", "spending", "income", "transfer", "pending"] as const).map((k) => (
          <button
            key={k}
            onClick={() => setKind(k)}
            aria-pressed={kind === k}
